...
COMMENT ON COLUMN companies.id IS '고객사 고유 식별자 (BIGINT, IDENTITY)';
COMMENT ON COLUMN companies.site_code IS '고객사 고유 사이트 코드 (URL 세그먼트용, 예: logcom, cheil)';
COMMENT ON COLUMN companies.name IS '고객사 공식 회사명';
COMMENT ON COLUMN companies.logo_url IS '고객사 헤더 노출용 로고 이미지 URL';
COMMENT ON COLUMN companies.primary_color IS '고객사 대표 브랜드 HEX 색상 코드 (예: #0052CC)';
...
COMMENT ON COLUMN departments.id IS '부서 고유 식별자 (BIGINT, IDENTITY)';
...
COMMENT ON COLUMN members.id IS '회원 고유 식별자 (BIGINT, IDENTITY)';
...
COMMENT ON COLUMN roles.id IS '역할 고유 식별자 (BIGINT, IDENTITY)';
COMMENT ON COLUMN roles.code IS '역할 고유 코드 (ROLE_SYSTEM_ADMIN, ROLE_COMPANY_ADMIN, ROLE_EMPLOYEE, ROLE_OPERATOR)';
...
COMMENT ON COLUMN templates.id IS '템플릿 고유 식별자 (BIGINT, IDENTITY)';
...
COMMENT ON COLUMN product_options.id IS '상품 옵션 고유 식별자 (BIGINT, IDENTITY)';
...
COMMENT ON COLUMN orders.id IS '주문 고유 식별자 (BIGINT, IDENTITY)';
...
COMMENT ON COLUMN order_snapshots.id IS '스냅샷 고유 식별자 (BIGINT, IDENTITY)';
...
COMMENT ON COLUMN shipments.id IS '배송 정보 고유 식별자 (BIGINT, IDENTITY)';
