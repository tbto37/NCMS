...
### 3.1 `companies` (고객사)
- `id` (BIGINT / int8, PK, IDENTITY)
- `site_code` (VARCHAR(50), UNIQUE) - URL 세그먼트용 (예: `logcom`, `cheil`)
- `name` (VARCHAR(100)) - 회사명
- `logo_url` (VARCHAR(500)) - 로고 이미지 경로
- `primary_color` (VARCHAR(10)) - 대표 HEX 색상 (예: `#0052CC`)
- `status` (VARCHAR(20)) - `ACTIVE`, `INACTIVE`
- `created_at`, `updated_at` (TIMESTAMPTZ)

### 3.2 `members` (회원)
- `id` (BIGINT / int8, PK, IDENTITY)
- `company_id` (BIGINT / int8, FK, NULLable - 내부 운영자는 NULL 가능)
- `department_id` (BIGINT / int8, FK, NULLable)
- `username` (VARCHAR(50), UNIQUE) - 로그인 아이디
- `password` (VARCHAR(255)) - Encoded 비밀번호
- `name` (VARCHAR(50)) - 이름
...
- **`roles`**:
  - `id` (BIGINT / int8, PK, IDENTITY)
  - `code` (VARCHAR(50), UNIQUE) - 역할 고유 코드 (`ROLE_SYSTEM_ADMIN`, `ROLE_COMPANY_ADMIN`, `ROLE_EMPLOYEE`, `ROLE_OPERATOR`)
  - `name` (VARCHAR(50)) - 역할명 (예: `시스템 관리자`)
  - `description` (VARCHAR(255)) - 역할 상세 설명 및 권한 범위
- **`member_roles`**:
  - `member_id` (BIGINT / int8, FK, PK) - 회원 ID
  - `role_id` (BIGINT / int8, FK, PK) - 역할 ID (`roles.id`)

### 3.3 `orders` (주문)
- `id` (BIGINT / int8, PK, IDENTITY)
- `order_no` (VARCHAR(50), UNIQUE) - 주문번호
- `company_id` (BIGINT / int8, FK) - 주문 고객사 ID
- `member_id` (BIGINT / int8, FK) - 주문자 ID
...
- `id` (BIGINT / int8, PK, IDENTITY)
- `order_id` (BIGINT / int8, FK, UNIQUE) - 주문 ID
- `card_data` (JSONB) - 주문 시 입력한 명함 문구 (한글/영문 이름, 부서, 직급, 전화번호 등)
- `product_option_summary` (VARCHAR(200)) - 재질명, 주문 수량 등 옵션 요약
- `preview_front_url`, `preview_back_url` (VARCHAR(500)) - 미리보기 이미지 식별자
- `print_pdf_url` (VARCHAR(500)) - 인쇄용 PDF 파일 식별자
- `created_at` (TIMESTAMPTZ)

### 3.5 `shipments` (배송 정보)
- `id` (BIGINT / int8, PK, IDENTITY)
- `order_id` (BIGINT / int8, FK, UNIQUE) - 주문 ID
- `carrier_code` (VARCHAR(50)) - 택배사 코드
...
## 4. 데이터 모델링 특징
- **PK**: 식별자는 `int8 GENERATED BY DEFAULT AS IDENTITY` (Long Auto Increment) 사용.
