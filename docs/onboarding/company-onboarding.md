...
| 고객사 로그인 화면에서 로그컴 어드민 계정 로그인 불가 오류 발생 | 로그컴 어드민 계정은 고객사 로그인 URL(`/{siteCode}/login`)에서 로그인 불가 | 로그컴 어드민 전용 URL(`/login`)에서 로그인 진행 |

---

## 4. 고객사 간 엄격한 로그인 권한 격리 (Tenant Security Policy)

1. **고객사 계정 (제일, 한미 등)**:
   - 본인 소속 고객사 전용 로그인 페이지(예: `/cheil/login`, `/hanmi/login`)에서만 로그인이 허용됩니다.
   - 타 고객사 로그인 페이지 및 로그컴 대표 어드민 페이지(`/login`) 접속 시 즉시 로그인 권한 에러와 함께 차단됩니다.
2. **로그컴 시스템 관리자 계정 (`ROLE_OPERATOR`)**:
   - 로그컴 대표 어드민 페이지(`/login`)에서만 로그인이 허용됩니다.
   - 타 고객사 로그인 페이지(`/{siteCode}/login`)에서 로그인 시도 시 **"로그컴 어드민 전용 계정입니다. 로그컴 어드민 페이지(/login)에서 로그인해 주세요."** 에러 메시지와 함께 로그인이 엄격히 차단됩니다.
