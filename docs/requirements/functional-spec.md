...
- **DAT-001 제일엔지니어링 & 한미글로벌 부서/직책/자격사항 테넌트별 동적 셀렉트박스 분리 및 1:1 영문 자동 완성**: 템플릿 편집 모달(`TemplateEditModal.tsx`) 내 하드코딩된 레거시 매핑을 완벽 제거하고, 선택된 고객사(`templateId` 기준 `cheil` / `hanmi`)에 따라 제공받은 실물 엑셀표 시트 데이터셋([companyData.ts](file:///c:/NCMS/frontend/src/shared/constants/companyData.ts))이 각각 분리 렌더링되도록 구현. 셀렉트박스에서 국문 부서/직급/자격사항 선택 시 영문 명함 문구가 고객사별로 1:1 자동 세팅 완성되는 기능 제공.
