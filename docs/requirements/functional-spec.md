...
- **SHP-004 택배사 송장 엑셀 업로드 자동 매칭**: 운영자 주문 화면 상단 `[송장 엑셀 업로드]` 버튼 클릭 시, 엑셀 파일 내 **`주문번호` + `이름(수령인/주문자)`** 2개 조건이 동시 일치하는 주문건들만 송장번호가 자동 일괄 입력되고 상태가 `발송완료(SHIPPED)`로 일괄 전환.
...
- **DAT-001 제일엔지니어링 & 한미글로벌 부서/직책 실데이터 1:1 영문 자동 완성**: `cheil.xlsx` 및 `hanmi.xlsx` 엑셀 기반 데이터셋([companyData.ts](file:///c:/NCMS/frontend/src/shared/constants/companyData.ts))을 구축하여 명함 입력 폼 및 템플릿 편집 모달에서 국문 부서/직책/자격사항 선택 시 영문 명함 문구가 1:1로 자동 세팅 완성되는 기능 제공.
