...
- **ORD-003 배송지 입력 및 주문 제출**: `[본사입고]` 원클릭 버튼(주소 자동 세팅), 신주소/구주소 자동완성 검색, 공사현장용 `수기 직접 입력` 지원, 주소 입력 바로 하단 배송/주문 메모 영역 배치 및 작성 메모 백엔드 스냅샷 연동 (`PENDING` 상태 시작, 5자리 레거시 주문번호 `00001` 자동 발급).
