...
- **ORD-003 배송지 입력 및 주문 제출**: `[본사입고]` 원클릭 버튼(주소 자동 세팅), 카카오/다음(Daum) 우편번호 API 연동 실제 한국 표준 주소 검색, 공사현장용 `수기 직접 입력` 지원, 주소 입력 하단 배송/주문 메모 영역 한 줄 전체(Full Width) 확장 배치 및 작성 메모 백엔드 스냅샷 연동 (`PENDING` 상태 시작, 5자리 레거시 주문번호 `00001` 자동 발급).

### 4.6 명함 검수 & 상태 변경 (Approval & Operator) - [완료]
- **APR-001 로그컴 운영자 검수**: `PENDING` 주문 목록 조회 및 명함 미리보기 확인.
- **APR-002 검수 승인/반려 및 자동 메일 발송**: 검수 승인(`APPROVED`) 시 `logcom2@naver.com`으로 `[제일엔지니어링] 주문이 승인되었습니다.` 자동 알림 메일 발송. 반려(`REJECTED`) 처리 시 반려 사유 필수 기재.
