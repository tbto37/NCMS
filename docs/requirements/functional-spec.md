...
- **APR-003 템플릿 사양 고도화**:
  - **제일엔지니어링**: 상호 폰트 `HY울릉도M`, 슬로건 `"Smiling Technology"` (`a파도소리` 8pt K80 `#333333`, 정밀 조절된 -7° `skewX` 기울임 적용), 직통번호 유무에 따른 수직 줄간격(Line Spacing) 동적 Y좌표 가변 연산 적용, 2번째 명함 템플릿(앞면 회사명 이미지 자산 `cheil_front_name.jpg` 적용 및 `cheil_build_front_name.jpg`와 상호 선택 가능) 지원 추가 완료.
