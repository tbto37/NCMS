...
- **APR-003 템플릿 사양 고도화**:
  ...
  - **한미글로벌**: 상호 폰트, 영문 주소 Y좌표 정밀 하향 조정, 연락처 `+82 (0)70-XXXX-XXXX` / `+82 (0)10-XXXX-XXXX` 국가번호 자동 주입 포맷팅, 앞면 주소 엔터(\n) 입력 시 2줄 자동 줄바꿈 분할 렌더링, 앞면 및 뒷면 우측 컨텐츠 블록 X위치 동기화(`x: 268`), 뒷면(영문) 로고 우측 이동(`x: 28`), SVG 내 독립 `<defs><style>` 프리텐다드 웹폰트 패스 자동 임베딩 및 다크모드 반전 방지 처리 적용 완료.
  - `승인대기`: 주문승인 (`APPROVED`), 주문반려 (`REJECTED`) (단, `ROLE_EMPLOYEE` 권한 계정은 `주문 취소`만 노출, `주문 승인` 및 `주문 반려` 비노출)
  - `승인완료`: 인쇄 시작 (`PRINTING`), 주문취소 (`CANCELLED`) (단, `ROLE_EMPLOYEE` 권한 계정은 `인쇄중` 버튼 비노출)
  - `인쇄중`: 발송 처리 (`SHIPPED`)
  - `발송완료`: (하단 액션 없음)
  - `승인반려`: 재승인 요청 (`PENDING`), 주문취소 (`CANCELLED`)
  - `주문취소`: 영구 삭제 (`DELETE`)
  - **컨펌 모달 & 승인 버튼 효과**: 선택 주문번호 및 **주문자 이름(예: 홍길동)** 명시, 긍정 파랑 테마(`bg-blue-600`) / 부정 빨강 테마(`bg-red-600`) 버튼 제공 및 상태 변경 후 해당 탭으로 자동 전환 (`ROLE_OPERATOR`, `ROLE_COMPANY_ADMIN` 계정은 주문 목록 체크박스 선택 시 `[주문 승인]` 버튼이 파란색(`bg-blue-600`)으로 강조되고 시각적 플래시/펄스 애니메이션(`animate-pulse`) 노출).
