...
- **QRY-001 주문 목록 & 상세 조회**: 역할별(임직원/기업관리자/운영자/시스템관리자) 권한에 맞는 주문 내역 및 배송 상태 조회 (테이블 1번 컬럼 `접수일자`, 2번 컬럼 `주문번호(00001)`, 주문 목록 '승인반려' 태그 클릭 시 반려 사유 팝업 노출, 주문 상세 모달 내 접수일자를 목록과 동일한 `YYYY-MM-DD (요일) HH:mm` 포맷으로 표시).
