...
- **QRY-001 주문 목록 & 상세 조회**: 역할별(임직원/기업관리자/운영자/시스템관리자) 권한에 맞는 주문 내역 및 배송 상태 조회 (주문 목록 '이름' 컬럼 및 상세 모달 이름에 배송지 수령인 이름(`recipientName`) 표시, 상세 모달 이메일 항목에 주문자 회원 이메일(`memberEmail`) 연동, '승인반려' 태그 마우스 오버 시 반려 사유 툴팁 노출, 상세 모달 접수일자를 `YYYY-MM-DD (요일) HH:mm` 포맷으로 표시).
