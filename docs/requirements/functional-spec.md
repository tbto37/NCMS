...
### 2.3 단순화된 권한 및 역할 체계 (RBAC)
- `EMPLOYEE` (일반 임직원): 템플릿 선택, 명함 입력/미리보기, 교정 동의, 주문 접수, 본인 주문 조회, 재주문
- `COMPANY_ADMIN` (기업 관리자): 소속 임직원 계정 등록·수정·중지, 부서 관리, 소속사 주문 목록 조회 (승인 권한 없음)
- `OPERATOR` (로그컴 운영자): 명함 오타/오류 검수(`APPROVED`/`REJECTED`), 인쇄 파일 확인/다운로드, 제작 및 배송 상태 변경, 송장 등록
- `SYSTEM_ADMIN` (시스템 관리자): 고객사 등록/수정, 템플릿/상품 옵션 관리 (※ 임직원 계정 등록은 기업 관리자만 가능)
...
### 4.5 명함 편집 & 교정확인 & 주문 (Order)
- **ORD-001 명함 입력 및 실시간 미리보기**: 템플릿 기반 명함 폼(이름, 직급, 부서, 연락처, 이메일 등) 입력 및 미리보기 연동.
- **ORD-002 교정 동의**: 오탈자, 연락처 확인 체크박스 동의 후 주문 진행.
- **ORD-003 주문 제출 & 스냅샷**: 수령지 정보 입력 후 주문 제출 시 당시 명함 문구/옵션 정보를 스냅샷으로 저장 (`PENDING` 상태 시작).

### 4.6 명함 검수 & 상태 변경 (Approval & Operator)
...
- **APR-004 반려건 수정/재주문**: 반려된 주문을 주문자가 수정하여 재주문 접수.

### 4.7 제작 & 배송 관리 (Production & Shipment)
- **SHP-001 인쇄중/발송 처리**: 운영자가 주문 상태를 `PRINTING`으로 변경하고 인쇄용 PDF 확인.
- **SHP-002 송장 입력 & 발송 완료**: 택배사 및 송장번호를 등록하여 `SHIPPED` 상태로 변경.
- **SHP-003 주문 영구 삭제**: 취소된 주문 내역 영구 삭제 처리 (`DELETE`).
- **QRY-001 주문 목록 & 상세 조회**: 역할별(임직원/기업관리자/운영자/시스템관리자) 권한에 맞는 주문 내역 및 배송 상태 조회.
