...
- **APR-002 검수 승인/반려 및 자동 메일 발송**: 업체관리부/운영자 검수 승인(`APPROVED`) 시 수신처 `logcom2@naver.com`으로 주문 승인 알림 메일 자동 발송 연동. 제일엔지니어링(`cheil`) 소속 `cheil_emp`(또는 소속 임직원)가 신규 명함 주문을 접수할 경우, `cheil_admin` 기업 관리자의 이메일 주소로 주문 완결/접수 안내 메일을 즉시 자동 발송 연동. 반려(`REJECTED`) 처리 시 반려 사유 필수 기재.
- **APR-003 템플릿 사양 고도화**:
