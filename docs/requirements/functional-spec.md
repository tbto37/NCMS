...
- **APR-003 템플릿 사양 고도화**:
  - **제일엔지니어링**: 상호 폰트 `HY울릉도M`, 슬로건 `"Smiling Technology"` (`a파도소리` 8pt K80 `#333333`, 정밀 조절된 -7° `skewX` 기울임 적용), 직통번호 유무에 따른 수직 줄간격(Line Spacing) 동적 Y좌표 가변 연산 적용 완료.
...
- **DAT-001 제일엔지니어링 & 한미글로벌 부서/직책/자격사항 실데이터 1:1 영문 자동 완성**: 제공받은 실물 엑셀표 시트 데이터셋([companyData.ts](file:///c:/NCMS/frontend/src/shared/constants/companyData.ts))을 1:1 정밀 검증 및 보강(`Railroad Business Div.`, `Assistant Manager`, `Engineer Railway`, `교통공학박사/교통기술사`, `Domestic Division`, `Division Leader` 등)하여 명함 입력 폼 및 템플릿 편집 모달 셀렉트박스에서 국문 선택 시 영문 명함 문구가 1:1로 정확하게 자동 세팅 완성되는 기능 제공.
