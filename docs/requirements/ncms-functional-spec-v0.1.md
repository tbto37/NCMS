...
- 로그컴 도메인 하위의 `/{고객사코드}` 경로 기반 고객사별 사이트 제공 (레거시 URL 방식 승계)
- 기업 고객사·부서·회원·권한 관리
...
- 명함 검수(승인·반려)는 로그컴 운영자 업무다. 고객사는 승인 절차 없이 주문만 접수하고 상태를 조회한다.
- 인쇄 및 발송 상태 처리는 로그컴 운영자 업무다.
- 고객사 임직원 계정 등록은 기업 관리자 업무다. 로그컴 시스템 관리자는 등록 권한이 없고 수정·중지만 할 수 있다.
- 고객사·템플릿·상품·운영계정 관리는 시스템 관리자 업무다.
- 단순 UI 숨김에 의존하지 않고 백엔드 API에서 권한과 고객사 범위를 검증한다.

### 3.5 경로 기반 고객사 사이트

- 고객사 대상 화면은 로그컴 도메인 하위 경로 `/{고객사코드}`로 접속한다. 예: `https://<로그컴도메인>/samsung`.
- 이 방식은 레거시 시스템의 URL 제공 방식을 그대로 승계한다. 고객사별로 소스나 배포본을 복제하지 않고, 하나의 애플리케이션이 경로의 고객사 코드로 대상 고객사를 식별한다.
- 고객사 코드(사이트 코드)는 고객사마다 유일하며 임의로 변경되지 않도록 관리한다.
- 경로의 고객사 코드는 로그인 화면·브랜딩(로고·대표색) 구성 기준으로 사용하고, 실제 데이터 접근 권한은 로그인 사용자의 `company_id`와 역할로 서버에서 다시 판정한다. URL을 다른 고객사 경로로 바꿔 접근해도 본인 소속 고객사 데이터만 취급되어야 한다.
- 로그컴 운영자·시스템 관리자용 내부 화면은 고객사 코드 경로 없이 별도 경로로 접속한다.

## 4. 사용자 역할 및 권한

| 역할 코드 | 역할명 | 주요 권한 |
|---|---|---|
| `EMPLOYEE` | 일반 임직원 | 템플릿 선택, 본인 명함 편집·주문, 본인 주문 조회·상태 확인, 반려 주문 수정, 재주문 |
| `COMPANY_ADMIN` | 기업 관리자 | 소속 고객사 임직원 계정의 등록·수정·중지, 부서 관리, 주문 조회, 엑셀 다운로드 (승인 권한 없음) |
| `OPERATOR` | 로그컴 운영자 | 전체 또는 배정 고객사 주문 조회, 명함 검수(승인·반려), 인쇄 파일 처리, 제작 상태 변경, 송장 등록, 발송 처리 |
| `SYSTEM_ADMIN` | 시스템 관리자 | 고객사·템플릿·상품 옵션·운영계정·정책 관리, 회원 정보 수정·중지(등록은 불가) |
...
| 명함 검수(승인·반려) | X | X | O | 예외 처리 |
| 인쇄중·발송완료 처리 | X | X | O | 예외 처리 |
| 회원 등록 | X | 소속사 | X | X (등록 불가) |
| 회원 수정·중지, 부서 관리 | X | 소속사 | 조회 | 수정·중지만 |
| 고객사 관리 | X | X | 조회 | O |
...
| 코드 | 명칭 | 설명 |
|---|---|---|
| `NOT_REQUIRED` | 검수 불필요 | 정책상 로그컴 검수 없이 바로 접수로 진행 |
| `PENDING` | 검수 대기 | 주문 제출 후 로그컴 운영자의 명함 검수 대기 |
| `APPROVED` | 검수 승인 | 로그컴 운영자가 명함 내용(오타 등)을 검수·승인함 |
| `REJECTED` | 반려 | 로그컴 운영자가 오타·오류 등 사유와 함께 반려함 |
...
- 주문 제출 후 로그컴 운영자가 명함 내용을 검수하고 승인 또는 반려한다. 고객사는 검수 주체가 아니다.
- 승인 반려 시 반려 사유(오타·오류 등)를 필수 입력한다.
...
| COM-007 | 고객사 사이트 코드·경로 | 고객사별 유일한 사이트 코드를 지정하고 `/{고객사코드}` 접속 경로를 제공한다. 레거시 URL 방식을 승계한다. | MVP |
...
임직원 계정 등록은 각 고객사 사이트(`/{고객사코드}`)에서 기업 관리자가 직접 수행한다. 로그컴 시스템 관리자는 신규 회원을 등록할 수 없고, 기존 회원의 정보 수정과 사용중지만 처리한다.

| ID | 기능 | 정의 | 우선순위 |
|---|---|---|---|
| MEM-001 | 회원 목록 | 아이디, 이름, 부서, 권한, 상태 기준으로 회원을 조회한다. | MVP |
| MEM-002 | 회원 등록 | 기업 관리자가 소속 고객사 사이트에서 아이디 중복 확인 후 이름, 부서, 연락처, 이메일, 역할을 등록한다. 로그컴 시스템 관리자는 이 기능을 사용할 수 없다. | MVP |
| MEM-003 | 회원 수정 | 기업 관리자 또는 로그컴 시스템 관리자가 회원 기본정보, 소속 부서, 역할, 사용 상태를 수정한다. | MVP |
| MEM-004 | 회원 사용중지 | 기업 관리자 또는 로그컴 시스템 관리자가 퇴사·이용 중지 회원의 로그인을 차단하고 이력은 보존한다. | MVP |
| MEM-005 | 비밀번호 초기화 | 관리자 화면에서 비밀번호를 표시하지 않고 초기화한다. | MVP |
| MEM-006 | 회원 일괄 등록 | 기업 관리자가 엑셀 양식으로 회원을 등록하고 오류 행을 안내한다. | 2차 |
...
| ORD-010 | 주문 제출 | 주문 스냅샷을 생성하고, 로그컴 검수 정책에 따라 검수 대기(`PENDING`) 또는 접수(`RECEIVED`) 상태로 전환한다. | MVP |
| ORD-011 | 주문 취소 | 허용 상태에서 주문을 취소하고 사유와 이력을 남긴다. | MVP |
| ORD-012 | 가격 표시 | 고객사 계약에 따라 예상 주문 금액을 표시한다. | 정책 확정 필요 |

### 6.11 명함 검수 (승인·반려)

로그컴 운영자가 주문된 명함의 오타·오류 등 내용을 검수하고 승인 또는 반려한다. 고객사는 이 단계에 관여하지 않고 결과 상태만 조회한다.

| ID | 기능 | 정의 | 우선순위 |
|---|---|---|---|
| APR-001 | 검수대기 목록 | 로그컴 운영자가 검수 대기(`PENDING`) 주문을 조회한다. | MVP |
| APR-002 | 주문 상세 확인 | 주문자, 명함 미리보기, 배송지, 상품 옵션, 메모를 확인한다. | MVP |
| APR-003 | 개별 승인 | 명함 검수 후 한 건의 주문을 승인한다. | MVP |
| APR-004 | 일괄 승인 | 선택한 여러 주문을 일괄 승인한다. | MVP |
| APR-005 | 반려 | 오타·오류 등 반려 사유를 필수 입력해 주문을 반려한다. | MVP |
| APR-006 | 반려 후 재주문 | 주문자가 반려 사유를 확인하고 내용을 수정해 교정 확인 후 다시 주문한다. | MVP |
| APR-007 | 검수 이력 | 승인자·승인일시·반려자·반려일시·반려 사유를 기록한다. | MVP |
| APR-008 | 검수 알림 | 승인 또는 반려 결과를 주문자에게 알린다. | MVP |
...
| ID | 알림 시점 | 수신자 | 우선순위 |
|---|---|---|---|
| NTF-001 | 주문 접수·검수 요청 | 로그컴 운영자 또는 운영 메일 | MVP |
| NTF-002 | 검수 승인 완료 | 주문자 | MVP |
| NTF-003 | 검수 반려 | 주문자 | MVP |
...
고객사 대상 화면(임직원·기업 관리자)은 모두 고객사 코드 경로 `/{companyCode}` 하위에 둔다. 로그컴 내부 화면(운영자·시스템 관리자)은 고객사 코드 없이 접속한다.
...
/{companyCode}/editor/:templateId
/{companyCode}/proof/:draftId
/{companyCode}/checkout/:draftId
/{companyCode}/orders
/{companyCode}/orders/:orderId
/{companyCode}/orders/:orderId/reorder
```

### 10.2 기업 관리자

기업 관리자는 승인 화면을 갖지 않는다. 회원 등록·수정·중지는 기업 관리자만 수행하는 고객사 사이트 전용 기능이다.

```text
/{companyCode}/company/orders
/{companyCode}/company/members
/{companyCode}/company/members/new
/{companyCode}/company/departments
```
...
시스템 관리자 화면에는 회원 등록 기능을 두지 않는다. 회원은 조회·수정·중지만 가능하다.
...
### 11.1 1차 최우선 착수 범위 (이번 요청)

발주사 요청에 따라 다음을 최우선으로 개발한다. 전체 흐름은 일반 이커머스 주문 프로세스와 유사한 B2B 구조다. 고객사가 주문을 넣고, 로그컴이 명함을 검수(승인/반려)한 뒤 인쇄·발송하며, 고객사는 주문 상태를 조회한다.

- 로그컴 도메인 하위 `/{고객사코드}` 경로 기반 고객사별 사이트 제공 (레거시 승계)
- 고객사 사이트에서 기업 관리자가 소속 임직원 계정을 직접 등록·수정·중지
- 고객사 임직원의 명함 발주: 템플릿 선택 → 간단편집 → 교정확인 → 주문 접수
- 고객사의 주문 상태 조회 (검수 승인 여부 확인 포함)
- 로그컴 운영자의 주문 확인·검수: 주문 목록·상세 조회, 명함 스냅샷 미리보기, 오타·오류 검수 후 승인 또는 반려
- 로그컴 운영자의 주문 상태 변경: 접수 → 인쇄중 → 발송 완료 등 제작·배송 상태 처리
- 위 흐름에 필요한 로그인, 고객사·회원·템플릿 기준정보, 고객사 데이터 격리

고객사 내부에는 별도 승인 절차가 없다. 승인·반려는 로그컴 운영자의 명함 검수 업무이며, 반려 시 오타·오류 등 사유를 필수로 남긴다.

### 11.2 1차 오픈 필수

- 로그인·로그아웃·비밀번호 변경·초기화
- 고객사·부서·역할 관리, 기업 관리자의 회원 등록·수정·중지
- 로그컴 시스템 관리자의 회원 수정·중지(등록 제외)
- 고객사별 데이터 격리
...
- 교정 확인과 확인 이력
- 주문서 작성·접수·주문번호 생성
- 로그컴 운영자의 명함 검수(승인·반려)와 반려 후 재주문
- 주문 목록·검색·상세·상태별 조회
- 주문 당시 스냅샷과 재주문
...
- 택배사 API 배송상태 자동조회
- 통계 대시보드
- 고객사별 독립 도메인 또는 고도화된 브랜딩 (경로 기반 기본 사이트 제공은 1차 포함)
...
| 번호 | 결정 항목 | 주요 선택지 |
|---|---|---|
| P-01 | 명함 검수 주체 | (확정) 고객사 내부 승인 없음, 로그컴 운영자가 명함 검수(승인·반려) 수행 |
| P-02 | 검수 처리 방식 | 개별 검수 / 일괄 승인 / 일부 고객사 검수 생략(`NOT_REQUIRED`) 허용 여부 |
...
13. 로그컴 도메인 하위 `/{고객사코드}` 경로 기반 고객사 사이트 제공(레거시 승계)을 명시하고, 경로는 진입·브랜딩 식별용, 데이터 권한은 서버 `company_id` 판정으로 이원화한다.
14. 명함 검수(승인·반려)를 기업 관리자가 아닌 로그컴 운영자 업무로 정의한다. 고객사는 내부 승인 없이 주문·상태 조회만 수행하며, 검수는 명함 오타·오류 확인 목적이다.
15. 임직원 계정 등록 권한을 기업 관리자로 일원화한다. 로그컴 시스템 관리자는 신규 회원을 등록할 수 없고 수정·중지만 할 수 있다.
...
- URL의 고객사 코드를 다른 고객사 값으로 바꿔 접근해도 본인 소속 고객사 데이터만 취급되어야 한다.
- 로그컴 시스템 관리자 계정으로 신규 회원 등록 API를 직접 호출해도 서버에서 차단해야 한다. 등록은 기업 관리자만 수행할 수 있다.
...
- 명함 검수(승인·반려)는 로그컴 운영자만 수행할 수 있고, 고객사 사용자는 수행할 수 없어야 한다.
...
| v0.2 | 2026-07-23 | 로그컴 도메인 하위 `/{고객사코드}` 경로 기반 고객사 사이트 제공(레거시 승계)을 MVP로 확정하고, 1차 최우선 착수 범위(고객사 사이트 제공·명함 발주·주문 확인·주문 상태 변경)를 정의. 명함 검수(승인·반려)를 로그컴 운영자 업무로 재정의하고 고객사 내부 승인 절차를 제거. 임직원 계정 등록 권한을 기업 관리자로 일원화하고 로그컴 시스템 관리자는 수정·중지만 가능하도록 정의 |
