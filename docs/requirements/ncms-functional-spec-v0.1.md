...
| `COMPANY_ADMIN` | 기업 관리자 (선택) | 소속 고객사의 주문 조회, 회원·부서 관리, 엑셀 다운로드 (승인 권한 없음) |
...
| v0.2 | 2026-07-23 | 로그컴 도메인 하위 `/{고객사코드}` 경로 기반 고객사 사이트 제공(레거시 승계)을 MVP로 확정하고, 1차 최우선 착수 범위(고객사 사이트 제공·명함 발주·주문 확인·주문 상태 변경)를 정의. 명함 검수(승인·반려)를 로그컴 운영자 업무로 재정의하고 고객사 내부 승인 절차를 제거 |
